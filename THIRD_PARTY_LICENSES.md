...
本アプリ(将棋Vault)は、モバイル(iOS/Android)版でオンデバイス解析を
実現するため、以下のGPLライセンスの将棋エンジン/評価関数を**アプリに組み込んで
配布**する(方針: `docs/ios_engine_integration_research.md` §5 の選択肢A)。

このため、**本アプリ全体を GNU General Public License v3.0 (GPLv3) で配布**する。
アプリ全文ライセンスは [`LICENSE`](LICENSE) を参照。
